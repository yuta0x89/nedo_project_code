...
def main() -> None:
    ...
    output_texts = []
    for i in range(len(example['output'])):  #end of sentenseはいらない？
        #print(example["output"][i] )
        if example['text'][i] is not None: #for ja
            text = f"以下は、タスクを説明する指示です。要求を適切に満たす応答を書きなさい\n\n### 指示:\n{example['text'][i]}\n\n### 応答: \n{example['output'][i]}"
            output_texts.append(text)
       
        elif example["text_en"][i] is not None: #for dolly
            text = f"Below are instructions explaining the task. Write a response that properly satisfies the request.\n\n### instruction:\n{example['text_en'][i]}\n\n### 応答: \n{example['output_en'][i]}"
            output_texts.append(text)
