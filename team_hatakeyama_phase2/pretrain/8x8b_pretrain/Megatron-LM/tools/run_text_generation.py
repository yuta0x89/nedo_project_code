...
if __name__ == "__main__":
    ...
    #    if choice.item() == 0:
    #        try:
    result = generate_and_post_process(model)
    print_rank_0(result)
    print_rank_0(result[-1].cpu().numpy())
    print_rank_0(result[-1].shape)

    np.save('/storage5/shared/Nishijima/test_moe/3rd_tonyu_iter_800_3rd_8layers_megatron',result[-1].cpu().numpy()[0])
