...
MASTER_PORT=65000
NNODES=1
chmod +x /storage5/Megatron-LM/scripts/tsubame/moe_test_hatakeyama/4nodes/tanuki-4x11b.sh
...
for node in "${NODES[@]}"; do
    ...
    ssh -q $node "cd $SCRIPT_ROOT && \
        bash /storage5/Megatron-LM/scripts/tsubame/moe_test_hatakeyama/4nodes/tanuki-4x11b.sh $NODE_RANK
