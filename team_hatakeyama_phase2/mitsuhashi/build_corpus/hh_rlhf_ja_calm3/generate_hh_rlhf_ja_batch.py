...
json_file_path = os.path.join('./out', 'hh-rlhf-12k-ja-calm3-22b.jsonl')
skip_file_path = os.path.join('./out', 'hh-rlhf-12k-ja-calm3-22b.txt')
...
def build_prompt(row):
    messages = [{'from': 'user', 'value': 'あなたは親切なAIアシスタントです。以下の対話に続く適切な応答を書きなさい。'}]
    messages.extend(row['conversations'])
    messages = list(map(calm3_formatter, messages))
...
def process_batches(dataframe, batch_size=256):
    ...
    while processed_count < total_rows:
        ...
        for index, row in batch_df.iterrows():
            prompt, messages = build_prompt(row)
        ...
        try:
            ...
            # バッチ内でイテレーションして応答を処理
            for local_idx, response, prompt, messages, source in zip(indices, generated_responses, prompts, messages_list, sources):
                if response.outputs and len(response.outputs) > 0:
                    generated_text = response.outputs[0].text.strip()

                    data = {
                        'conversations': messages,
                        'chosen_orig': dataframe.at[local_idx, 'chosen'],
                        'chosen': generated_text,
                        'rejected': dataframe.at[local_idx, 'rejected'],
                        'source': source,
                        'chosen_generated_model': 'cyberagent/calm3-22b-chat',
                        'generated_prompt': [messages[0]],
                    }
