...
json_file_path = os.path.join('./out', 'openplatypus-ja-calm3-22b.jsonl')
skip_file_path = os.path.join('./out', 'openplatypus-ja-calm3-22b.txt')
...
def process_batches(dataframe, batch_size=256):
    ...
    while processed_count < total_rows:
        ...
        for index, row in batch_df.iterrows():
            if row['data_source']=='MATH/PRM-800K' or row['data_source']=='ARB' or row['data_source']=='scibench':
        ...
        try:
            ...
            for local_idx, response, prompt, row in zip(indices, generated_responses, prompts, rows):
                if response.outputs and len(response.outputs) > 0:
                    ...
                    instruction = generated_text.split('\n\n### 答え\n')[0]
                    instruction = instruction.split('### 質問\n')[-1]

                    data = {
                        'idx': idx,
                        'instruction_en':f"{row['input']}{row['instruction']}",
