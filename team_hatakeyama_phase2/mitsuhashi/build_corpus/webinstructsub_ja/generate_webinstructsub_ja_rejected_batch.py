...
json_file_path = os.path.join('./out', 'WebInstructSub_ja_100k_1st.jsonl')
skip_file_path = os.path.join('./out', 'WebInstructSub_ja_100k_1st.txt')
...
dataset = load_dataset("ryota39/WebInstructSub_ja_100k_1st", split='train')
df = pd.DataFrame(dataset)

model_path = 'team-hatakeyama-phase2/8B-nishijima-tanuki8b_dpo_full_001-checkpoint-137'
...
def process_batches(dataframe, batch_size=256):
    ...
    while processed_count < total_rows:
        ...
        try:
            ...
            # バッチ内でイテレーションして応答を処理
            for local_idx, response, prompt, row, messages in zip(indices, generated_responses, prompts, rows, messages_list):
                if response.outputs and len(response.outputs) > 0:
                    ...
                    data = {
                        ...
                        'chosen_en': row['answer'],
                        'prompt': messages,
                        'chosen': row['chosen'],
                        'rejected': response,
                        'chosen_model': 'cyberagent/calm3-22b-chat',
                        'rejected_model': 'team-hatakeyama-phase2/8B-tanuki8b_dpo_full_001-checkpoint-137',
