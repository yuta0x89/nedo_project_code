...
def main() -> None:
    parser = HfArgumentParser((DPOConfig, DPOTrainingArguments, WandbArguments))
    training_args, dpo_training_args, wandb_args = parser.parse_args_into_dataclasses()
    ...
    #train_dataset = load_datasets(dpo_training_args.data_files)
    train_dataset = load_datasets(dpo_training_args.data_files, dpo_training_args.hf_cache_dir, dpo_training_args.train_split)
    if dpo_training_args.eval_data_files:
        print("do eval")
        #eval_dataset = load_datasets(dpo_training_args.eval_data_files)
        eval_dataset = load_datasets(dpo_training_args.eval_data_files, dpo_training_args.hf_cache_dir, dpo_training_args.eval_split)
    ...
    if dpo_training_args.model_name_or_path.find("mergoo")>=0:
    ...
    save_scripts = dpo_training_args.save_scripts.split(',') + [os.path.abspath(__file__)]
