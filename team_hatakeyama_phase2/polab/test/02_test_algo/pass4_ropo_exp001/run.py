...
parser.add_argument('--debug', action='store_true', help='デバッグ用にwandbの記録を全てpolab-debugに送る')
...
polab_dir = os.path.dirname(exp_project_dir)
polab_dir_name = os.path.basename(polab_dir)

# 実験共通となる設定のyamlファイルを読み取る
base_config_path = os.path.dirname(polab_dir) + '/base_config/base_config.yaml'
...
inst_path = exp_config['data']['name']
eval_path = inst_path

# wandbの設定
wandb_entity = base_config['wandb']['entity']
if args.debug:
    wandb_project = 'polab-debug'
else:
    wandb_project = f'polab-{polab_dir_name}'
    if polab_dir_name == 'experiments':
...
# tag名は64charまでの制限がある
wandb_tags = [exp_config['model']['name'][-64:], exp_config['data']['name'][-64:], args.exp_config, args.accelerate_config]
...
# モデルの保存先
if args.debug:
    output_dir = 'polab-debug'
else:
    output_dir = f'polab-{polab_dir_name}'
...
cmd = f"""{pre_cmd}  \
    ...
    --max_prompt_length {exp_params['max_prompt_length']} \
    --beta {exp_params['beta']} \
    --ropo_alpha {exp_params['ropo_alpha']} \
    --ropo_gamma {exp_params['ropo_gamma']} \
    ...
    --train_split {exp_config['data']['train_split']} \
    --eval_split {exp_config['data']['eval_split']} \
