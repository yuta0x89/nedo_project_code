...
class GPOTrainingArguments:
    ...
    def __post_init__(self):
        ...
        if self.peft_target_model == "tanuki-8x8-mini":
           self.peft_target_modules = [
                "q_proj",
                "k_proj",
                "v_proj",
...
def load_datasets(data_list, template_config, cache_dir, tokenizer, split):
    ...
    for data_config in data_list:
        if split in data_config['split']:
            ...
            if data_config['name'] == 'team-hatakeyama-phase2/0716-calm3-22b-random-genre-inst-dpo':
                for key in ['instruction', 'rejected', 'accepted']:
                    _ , unique_indices = np.unique(dataset[key], return_index=True, axis=0)
