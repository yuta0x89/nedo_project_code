...
def load_datasets(data_list, cache_dir, split):
    ...
    for data_config in data_list:
        ...
        if split in data_config['split']:
            ...
            colnames = data_config['colnames']
            def create_prompt(example): 
                prompt = f"以下は、タスクを説明する指示と、文脈のある入力の組み合わせです。要求を適切に満たす応答を書きなさい。\n\n### 指示:\n{example[colnames['instruction']]}"
                if 'input' in colnames and type(example[colnames['input']]) is str and len(example[colnames['input']]) > 0:
                    prompt += f"\n\n### 入力:\n{example[colnames['input']]}"
                prompt += f"\n\n### 応答:\n{example[colnames['output']]}"
                return prompt

            dataset = dataset.map(lambda example: {"text": create_prompt(example)})
...
def main() -> None:
    ...
    train_dataset = load_datasets(data_args.data_list, sft_training_args.hf_cache_dir, 'train')
    if training_args.do_eval:
        print("do eval")
        #eval_dataset = load_datasets(sft_training_args.eval_data_files)
        eval_dataset = load_datasets(data_args.data_list, sft_training_args.hf_cache_dir, 'eval')
