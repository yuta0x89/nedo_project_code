...
def convert_to_target(samples):
    targets = []
    for m in samples["messages"]:
        targets.append(m[0]["content"])
...
if "messages" in dataset.column_names:
    dataset = dataset.map(convert_to_target, batched=True)
    dataset_fields = ["target" if field == "messages" else field for field in dataset_fields]
    print(f"Dataset columns: {dataset.column_names}", flush=True)
    print(f"Dataset fields: {dataset_fields}", flush=True)
...
with open(output_tsv, "a") as f:
    ...
    for i in range(start, end, batch_size):
        ...
        batch = ["\n".join([str(dic[field]) or "" for field in dataset_fields]) for dic in dicts]
        scores = llm.ask(batch)
        for index, score in enumerate(scores.tolist()):
            line = f"{score:.4f}\t{batch_start + index}"
            f.write(line + "\n")
            f.flush()
            print(f"{line} {batch[index]}", flush=True)
