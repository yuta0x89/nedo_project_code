    ...
    for vp_rank in range(vp_size):
        ...
        for pp_rank in range(pp_size):
            ...
            for layer_num in range(len(get_transformer_block(models[0]).layers)):
                ...
                mlp_l0_bias = []
                mlp_l1_weight = [[] for _ in range(tp_size)]
                for tp_rank, model in enumerate(models):
                    ...
                    #print(layer.mlp)
                    mlp_l0_weight[tp_rank] = torch.chunk(layer.mlp.experts.weight1.data, md.num_experts, dim=1)
                    mlp_l1_weight[tp_rank] = torch.chunk(layer.mlp.experts.weight2.data, md.num_experts, dim=0)
                    if md.linear_bias:
