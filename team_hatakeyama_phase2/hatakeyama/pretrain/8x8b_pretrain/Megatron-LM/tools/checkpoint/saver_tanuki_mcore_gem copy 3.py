...
def save_checkpoint(queue, args):
    ...
    for pp_rank in range(args.target_pipeline_parallel_size):
        ...
        for layer in range(len(setter.get_transformer_block(models[0]).layers)):
            ...
            mlp_w2_weight = [None] * (md.num_experts)
            mlp_w13_weight = [None] * (md.num_experts)
            
            for expert_idx in range(md.num_experts):
                mlp_w1_weight, mlp_w3_weight = torch.chunk(msg.pop(f"mlp {expert_idx} weight1"), 2, dim=0) #2 * (ffn_hidden_size, hidden_size)
                mlp_w1_weight = mlp_w1_weight.transpose(0,1) # (hidden_size, ffn_hidden_size)
                mlp_w1_weight = torch.chunk(
                    mlp_w1_weight, args.target_tensor_parallel_size, dim=1) # tp * (ffn_hidden_size, hidden_size//tp)
                #print(f"temp_weight[0] :{temp_weight[0].shape}")
                #print(f"temp_weight[1] :{temp_weight[1].shape}")
                mlp_w3_weight = mlp_w3_weight.transpose(0,1) # (hidden_size, ffn_hidden_size)
                mlp_w3_weight = torch.chunk(
                    mlp_w3_weight, args.target_tensor_parallel_size, dim=1) # tp * (hidden_size, ffn_hidden_size//tp)
                mlp_w2_weight[expert_idx] = msg.pop(f"mlp {expert_idx} weight2").transpose(0, 1) # (ffn_hidden_size, hidden_size)
                mlp_w2_weight[expert_idx] = torch.chunk(
                    mlp_w2_weight[expert_idx], args.target_tensor_parallel_size, dim=0) #tp * (ffn_hidden_size//tp, hidden_size)
                mlp_w13_weight[expert_idx]  = [torch.cat(weights, dim=1) for weights in zip(mlp_w1_weight, mlp_w3_weight)]
                # tp * (hidden_size, ffn_hidden_size//tp*2)
            ...
            for tp_rank in range(args.target_tensor_parallel_size):
                for expert_ids in range(md.num_experts):
                    if expert_ids == 0:
                        mlp_expert_w13_weight[tp_rank]  = mlp_w13_weight[expert_ids][tp_rank] # tp * (hidden_size, ffn_hidden_size//tp*2)
                        mlp_expert_w2_weight[tp_rank] = mlp_w2_weight[expert_ids][tp_rank] #tp * (ffn_hidden_size//tp, hidden_size)
                    else:
                        mlp_expert_w13_weight[tp_rank] = torch.cat([mlp_expert_w13_weight[tp_rank], mlp_w13_weight[expert_ids][tp_rank]], dim=1)
                        # tp * (hidden_size, ffn_hidden_size//tp*2 * num_experts)
                        mlp_expert_w2_weight[tp_rank] = torch.cat([mlp_expert_w2_weight[tp_rank], mlp_w2_weight[expert_ids][tp_rank]], dim=0)
                        # tp * (ffn_hidden_size//tp * num_experts, hidden_size)
            ...
            for tp_rank in range(args.target_tensor_parallel_size):
                params_dict = {
                    ...
                    "mlp_expert_w13_weight" : mlp_expert_w13_weight[tp_rank],
                    "mlp_expert_w2_weight" : mlp_expert_w2_weight[tp_rank],
                }
                if md.norm_has_bias:
                    params_dict.update({
                        "self_attn_norm_bias" :
