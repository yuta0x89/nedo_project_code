...
def set_distributed_env():
    ...
    os.environ['WORLD_SIZE'] = '1'  # 総プロセス数
    os.environ['RANK'] = '0'  # 現在のプロセスのランク
    #os.environ['LOCAL_RANK'] = '6'  # ローカルのGPU番号
    os.environ['MASTER_ADDR'] = 'slurm0-a3-ghpc-6'  # マスターノードのアドレス
    os.environ['MASTER_PORT'] = '65011'  # マスターポートのアドレス
    print(f'set envioroment')
...
def set_postprocess_state(args, model, hf_model):
    ...
    model.language_model.encoder.final_norm.weight.data.copy_(hf_model.norm.weight)
    #model.language_model.encoder.final_norm.weight.data.copy_(hf_model.model.norm.weight)
    model.language_model.output_layer.weight.data.copy_(hf_model.norm.weight)
...
def set_mlp_state(args, layer, hf_layer):
    ...
    for idx in range(args.num_experts):
        ...
        #w1 = getattr(mlp.local_experts, f"{idx}").w1.weight.data.copy_
        #w2 = getattr(mlp.local_experts, f"{idx}").w2.weight.data.copy_
        #w3 = getattr(mlp.local_experts, f"{idx}").w3.weight.data.copy_
...
def _load_checkpoint(queue, args):
    ...
    mpu.set_expert_model_parallel_world_size(margs.expert_parallel_size)
