...
def init_method(tensor, sigma=0.02):
...
def initialize_rng_states(args):
    ...
    # args.seedが存在しない場合、デフォルト値を使用
    seed = getattr(args, 'seed', 1234)  # 1234はデフォルト値の例です
    rng_tracker.add(data_parallel_rng_name, seed)
    # 必要に応じて、他のRNG状態も追加
    # 例：expert_parallel_rng_name = mpu_random.get_expert_parallel_rng_tracker_name()
...
def save_checkpoint(queue, args):
    ...
    if md.true_vocab_size is not None:
        ...
        margs.make_vocab_size_divisible_by = args.target_tensor_parallel_size
        margs.padded_vocab_size = orig_vocab_size #_vocab_size_with_padding(md.true_vocab_size, margs)
        ...
        # Expanding embedding to larger size by replicating final entry
            ...
            padding_tensor = torch.randn(padding_size, orig_word_embed.shape[1])
            padding_tensor = init_method(padding_tensor)
            full_word_embed = torch.cat((orig_word_embed, padding_tensor))
    ...
    for pp_rank in range(args.target_pipeline_parallel_size):
        ...
        for layer in range(len(models[0].language_model.encoder.layers)):
            ...
            # args.target_tensor_parallel_sizeで分割するのでtp=2の場合、qkvqkvという順番になっている必要がある
        ...
        if post_process:
            ...
            if md.output_layer:
                ...
                if not hasattr(models[0].language_model, 'output_layer'):
                    ...
                    exit(1)
                hf_output_weight = msg.pop("weight")
                if padding_size is not None:
                    # paddingするときは、padding分のweightを追加してsize違いが起きないようにする
                    padding_tensor = torch.randn(padding_size, hf_output_weight.shape[1])
                    padding_tensor = init_method(padding_tensor)
                    hf_output_weight = torch.cat((hf_output_weight, padding_tensor))
                output_layer_weight = torch.chunk(hf_output_weight, args.target_tensor_parallel_size, dim=0)
                for tp_rank in range(args.target_tensor_parallel_size):
