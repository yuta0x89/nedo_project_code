...
def add_arguments(parser):
    ...
    group.add_argument('--target-pipeline-parallel-size', type=int,
                       ...
                       'in the input checkpoint if provided by the loader, otherwise to 1')
    group.add_argument('--target-expert-model-parallel-size', type=int, default=1,
                       help='Target expert model parallel size')
    group.add_argument('--saver-transformer-impl', default='transformer_engine',
                       choices=['local', 'transformer_engine'],
                       help='Which Transformer implementation to use.')
    group.add_argument('--sequence-parallel', action='store_false',
                       help='Target tensor model parallel size, default to the pipeline parall size '
                       'in the input checkpoint if provided by the loader, otherwise to 1')
...
class MCoreTESetter_v2(ModelSetter):
    ...
    def set_layer(cls, model, layer_idx, **kwargs):
        ...
        print(layer.mlp.experts.local_experts[0].linear_fc1)
        num_local_experts = kwargs['mlp_expert_w13_weight'].shape[0]
        for i in range(num_local_experts):
            expert = layer.mlp.experts.local_experts[i]
            cls.set_tensor(expert.linear_fc1.weight, kwargs['mlp_expert_w13_weight'][i])
            cls.set_tensor(expert.linear_fc2.weight, kwargs['mlp_expert_w2_weight'][i])
...
def save_checkpoint(queue, args):
    ...
    if hasattr (md, 'checkpoint_args'):
        ...
        args_to_keep = ['tensor_model_parallel_size', 'pipeline_model_parallel_size', 'world_size', 'params_dtype',
                        ...
                        'moe_aux_loss_coeff', 'transformer_impl', "bf16",  'use_mcore_models']
        #"moe_grouped_gemm",
    ...
    margs.sequence_parallel = True #md.checkpoint_args.sequence_parallel
    ...
    for pp_rank in range(args.target_pipeline_parallel_size):
        ...
        for layer in range(len(setter.get_transformer_block(models[0]).layers)):
            ...
            for expert_idx in range(md.num_experts):
                temp_weight = torch.chunk(msg.pop(f"mlp {expert_idx} weight1"), 2, dim=0)
                mlp_w1_weight = torch.chunk(
                    temp_weight[0], args.target_tensor_parallel_size, dim=0)
                #print(f"temp_weight[0] :{temp_weight[0].shape}")
                #print(f"temp_weight[1] :{temp_weight[1].shape}")
                
                mlp_w3_weight = torch.chunk(
                    temp_weight[1], args.target_tensor_parallel_size, dim=0)
            ...
            for tp_rank in range(args.target_tensor_parallel_size):
                for expert_ids in range(md.num_experts):
                    if expert_ids == 0:
                        mlp_expert_w13_weight[tp_rank] = mlp_w13_weight[expert_ids][tp_rank].unsqueeze(0)
                        mlp_expert_w2_weight[tp_rank] = mlp_w2_weight[expert_ids][tp_rank].unsqueeze(0)
                    else:
                        mlp_expert_w13_weight[tp_rank] = torch.cat([mlp_expert_w13_weight[tp_rank], mlp_w13_weight[expert_ids][tp_rank].unsqueeze(0)], dim=0)
                        mlp_expert_w2_weight[tp_rank] = torch.cat([mlp_expert_w2_weight[tp_rank], mlp_w2_weight[expert_ids][tp_rank].unsqueeze(0)], dim=0)
                #print(mlp_expert_w13_weight[tp_rank].shape)
