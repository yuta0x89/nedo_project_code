...
def _load_checkpoint(queue, args):
    ...
    md.num_experts_per_tok = margs.moe_router_topk
    ...
    for vp_rank in range(vp_size):
        ...
        for pp_rank in range(pp_size):
            ...
            for layer_num in range(len(get_transformer_block(models[0]).layers)):
                ...
                qkv_bias = []
                dense_weight = []
                mlp_l0_weight = [[] for _ in range(md.num_experts)]
                mlp_l0_bias = []
                mlp_l1_weight = [[] for _ in range(md.num_experts)]
                for tp_rank, model in enumerate(models):
                    ...
                    for expert_ids in range(md.num_experts):
                        mlp_l0_weight[expert_ids].append(layer.mlp.experts.local_experts[expert_ids].linear_fc1.weight.data)
                        mlp_l1_weight[expert_ids].append(layer.mlp.experts.local_experts[expert_ids].linear_fc2.weight.data)
                    if md.linear_bias:
                ...
                if md.swiglu:
                    # Process mlp_l0_weight
                    for expert_ids in range(md.num_experts):
                        for tp_rank in range(len(mlp_l0_weight[expert_ids])):
                            mlp_l0_weight[expert_ids][tp_rank] = torch.chunk(mlp_l0_weight[expert_ids][tp_rank], 2, dim=0)

                    # Concatenate W1 and W3 weights
                    for i in range(md.num_experts):
                        message[f"expert_{i} W1 weight"] = torch.cat([w[0] for w in mlp_l0_weight[i]], dim=0)
                        message[f"expert_{i} W3 weight"] = torch.cat([w[1] for w in mlp_l0_weight[i]], dim=0)

                    # concat all the first halves ('W's) and all the second halves ('V's)
                    #for expert_ids in range (md.num_experts):
                    #    for tp_rank in range(tp_size):
                    #        mlp_l0_weight[expert_ids][tp_rank] = torch.chunk(mlp_l0_weight[expert_ids][tp_rank], 2, dim=0)
                    #    message[f"expert_{i} W1 weight"] = torch.cat([w[0][expert_ids] for w in mlp_l0_weight], dim=0)
                    #    message[f"expert_{i} W3 weight"] = torch.cat([w[1][expert_ids] for w in mlp_l0_weight], dim=0)
                else:
                ...
                for expert_ids in range (md.num_experts):
                    message[f"expert_{expert_ids} W2 weight"] = torch.cat(mlp_l1_weight[expert_ids], dim=1)
                if md.linear_bias:
