...
class GPTDatasetConfig(BlendedMegatronDatasetConfig):
    ...
    """
    with_packing: bool = False      ##modify
...
class GPTDataset(MegatronDataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if (
            ...
            attention_mask, loss_mask, position_ids = _get_ltor_masks_and_position_ids(
                ...
                self.config.eod_mask_loss,
                self.config.create_attention_mask,
                self.config.with_packing                #modify
            )
            if self.masks_and_position_ids_are_cacheable:
                self.cached_attention_mask = attention_mask
                self.cached_loss_mask = loss_mask
    ...
    def _query_document_sample_shuffle_indices(
        ...
        ### <<< modify <<< ###
        if self.config.with_packing is False :
        ...
        elif self.config.with_packing is True :
            doc_index, _ = self.sample_index[idx]
            sample_parts = self.dataset.get_with_packing(
                            self.document_index[doc_index],
                            offset=None,
                            set_length=self.config.sequence_length, )

            return numpy.array((sample_parts), dtype=numpy.int64), None
        ### >>> modify >>> ###
    ...
    def _build_document_sample_shuffle_indices(
        ...
        if not cache_hit and (
            ...
            if self.config.with_packing is False :
                ...
                print(" ----- Packing Mode On ---- ")
                doc_num = numpy.arange( len(document_index)-1 )  # Includes the end value, hence 79947366
                offset_num = numpy.zeros_like(doc_num)
                sample_index = numpy.column_stack((doc_num, offset_num))
            ### >>> modify >>> ###
...
def _get_ltor_masks_and_position_ids(
    ...
    ### <<< modify <<< ###
    if with_packing is True :
        pad_token = 3
        loss_mask[data == pad_token] = 0.0
        attention_mask = attention_mask.squeeze()
        indices_of_three = (data == 3).nonzero(as_tuple=True)[0]
        if len(indices_of_three) > 0:
            first_index_of_three = indices_of_three[0].item()
            attention_mask[ :, first_index_of_three:] = 0
        else:
            first_index_of_three = -1
