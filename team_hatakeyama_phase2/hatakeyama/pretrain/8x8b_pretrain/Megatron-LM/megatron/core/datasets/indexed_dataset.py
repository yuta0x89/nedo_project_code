...
class IndexedDataset(torch.utils.data.Dataset):
    ...
    ### <<< modify <<< ###
    def get_with_packing(self, idx: int, offset: int = 0, set_length: Optional[int] = None) -> numpy.ndarray:
        """Retrieve a single item from the dataset with the option to only
        return a portion of the item with packing.
        ...
        if self.bin_buffer:
            sequence = numpy.frombuffer(
                self.bin_buffer, dtype=self.index.dtype, count=sequence_length, offset=sequence_pointer
            )
        else:
            sequence = numpy.empty(sequence_length, dtype=self.index.dtype)
            with open(get_bin_path(self.path_prefix), mode='rb', buffering=0) as bin_buffer_file:
        ...
        output_length = set_length + 1
        if sequence.shape[0] == output_length:
            pass
        elif sequence.shape[0] > output_length :
            sequence = sequence[:output_length]
        elif sequence.shape[0] < output_length :
            sequence = numpy.pad(sequence, (0, output_length - len(sequence)), constant_values=3)
