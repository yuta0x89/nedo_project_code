...
def track_moe_metrics(
    loss_scale, iteration, wandb_writer=None, total_loss_dict=None, per_layer_logging=False, wandb_stats=None,
    tensorboard_log_interval = 0, is_last_rank = False
):
    # Aux loss logging
    #print('start loss')
    aggregate_aux_losses_tracker_across_pipeline_parallel()
        
    if wandb_writer and (iteration % tensorboard_log_interval == 0) and is_last_rank:
        aux_losses = {k: v.float() * loss_scale for k, v in get_aux_losses_tracker().items()}
        #print(f"calculated aux_loss {aux_losses}")
        for name, loss_list in aux_losses.items():
            #print(f"{name} : {loss_list}")
            ...
            if wandb_writer:
                #print(f"{name}: {loss_list.mean()}")
                wandb_stats[f"{name}"] = loss_list.mean()
                #writer.add_scalar(name, loss_list.mean(), iteration)
                if per_layer_logging:
                    for i, loss in enumerate(loss_list.tolist()):
                        #print(f"moe/{name}_layer_{i}: {loss}")
    ...
    #print(f"finish calculated aux_loss")
    return wandb_stats
