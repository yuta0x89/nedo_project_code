...
skip_loss_threshold=11.21

##############################
##addiing loss spike handler##
##############################
class LossSpikeHandler:
    def __init__(self, threshold=1.2, window_size=10, max_spikes=1):
    ...
    def is_spike(self, loss):
        self.loss_history.append(loss)    
        print_rank_0(f"loss_history: {self.loss_history}")
        ...
        print_rank_0(f"avg_loss for check loss spike: {avg_loss}")
        print_rank_0(f"loss for check loss spike: {loss}")
        if loss > 2.5:#self.threshold * avg_loss:
            self.spike_count += 1
            return True
        return False

    def should_skip_optimization(self):
        return self.spike_count >= self.max_spikes

    def reset(self):
        self.spike_count = 0

##############################
##en of loss spike handler##
##############################
...
def train_step(forward_step_func, data_iterator,
               model, optimizer, opt_param_scheduler, config, spike_handler):
    ...
    ##############################
    if skip_loss_spike:
        skip_step = torch.zeros(1, device='cuda')
        if mpu.is_pipeline_last_stage(ignore_virtual=True):
            for key in losses_reduced[0]:
                losses_reduced_for_key_temp = [x[key] for x in losses_reduced]
                loss_reduced_temp = (sum(losses_reduced_for_key_temp) / len(losses_reduced_for_key_temp)).cpu().numpy()
                print(f"loss_reduced for skip detect = {loss_reduced_temp}")
                if loss_reduced_temp > skip_loss_threshold:
                    skip_step[0] = 1
        
        # Synchronize skip_step across all processes
        torch_distributed.all_reduce(skip_step, op=torch_distributed.ReduceOp.MAX)
        
        if skip_step[0] == 1:
            print(f"Loss exceeds maximum allowed value. Skipping step.")
            optimizer.zero_grad()
            return {}, 1, 0.0, 0  # Skipped iteration
...
def training_log(loss_dict, total_loss_dict, learning_rate, decoupled_learning_rate, iteration,
    ...
    if args.num_experts is not None:
        moe_loss_scale = 1 / get_num_microbatches()
        wandb_stats = track_moe_metrics(moe_loss_scale, iteration, wandb_writer, total_loss_dict, args.moe_per_layer_logging, wandb_stats, args.tensorboard_log_interval, is_last_rank())
    ...
    if wandb_writer and (iteration % args.tensorboard_log_interval == 0) and is_last_rank():
        ...
        #    moe_loss_scale = 1 / get_num_microbatches()
        #    wandb_stats = track_moe_metrics(moe_loss_scale, iteration, wandb_writer, total_loss_dict, args.moe_per_layer_logging, wandb_stats)
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    spike_handler = LossSpikeHandler()
