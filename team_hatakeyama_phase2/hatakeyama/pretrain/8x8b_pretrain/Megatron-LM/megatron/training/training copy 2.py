...
def train_step(forward_step_func, data_iterator,
    ...
    for key in losses_reduced[0]:
        ...
        #loss_reduced_temp[key] = (sum(losses_reduced_for_key) / len(losses_reduced_for_key)).cpu().numpy()
        loss_reduced_temp = (sum(losses_reduced_for_key) / len(losses_reduced_for_key)).cpu().numpy() #lossは一つのみ
    print(f"losses_reduced for avg: {losses_reduced}")
    print(f"losses_reduced_temp: {loss_reduced_temp}")
    if spike_handler.is_spike(loss_reduced_temp):
        print_rank_0(f"Loss spike detected: {loss_reduced_temp}. Current spike count: {spike_handler.spike_count}")
        if spike_handler.should_skip_optimization():
            print_rank_0("Max spikes reached. Skipping optimization and resetting.")
            spike_handler.reset()
            update_successful, grad_norm, num_zeros_in_grad = False, 0, 0
        else:
