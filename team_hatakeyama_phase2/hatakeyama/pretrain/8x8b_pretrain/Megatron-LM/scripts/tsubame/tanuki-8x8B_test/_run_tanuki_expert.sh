...
for node in "${NODES[@]}"; do
    ...
    ssh -q $node "source ~/miniconda3/etc/profile.d/conda.sh && \
        ...
        cd $SCRIPT_ROOT && \
        bash /storage5/Megatron-LM/scripts/tsubame/tanuki-8x8B/tanuki-4x8b-tp4_expert.sh $NODE_RANK" > /storage5/Megatron-LM/scripts/tsubame/tanuki-8x8B/log/${node}_output_${current_time=}.log 2>&1 &
    ((NODE_RANK+=1))
done
wait
