...
MASTER_PORT=65000
NNODES=16
chmod +x /storage5/Megatron-LM/scripts/tsubame/moe_test_hatakeyama/16nodes/tanuki-8x8b.sh
...
for node in "${NODES[@]}"; do
    ...
    ssh -q $node "cd $SCRIPT_ROOT && \
        bash /storage5/Megatron-LM/scripts/tsubame/moe_test_hatakeyama/16nodes/tanuki-8x8b.sh $NODE_RANK
