...
MASTER_PORT=65000
NNODES=1
chmod +x $SCRIPT_ROOT/scripts/tsubame/moe_test_hatakeyama/tanuki-4x8b_wo_zloss.sh
...
for node in "${NODES[@]}"; do
    ...
    ssh -q $node "cd $SCRIPT_ROOT && \
        bash $SCRIPT_ROOT/scripts/tsubame/moe_test_hatakeyama/tanuki-4x8b_wo_zloss.sh $NODE_RANK
