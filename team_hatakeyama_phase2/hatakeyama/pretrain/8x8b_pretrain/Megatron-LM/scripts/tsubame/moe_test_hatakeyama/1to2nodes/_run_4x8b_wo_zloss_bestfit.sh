...
MASTER_PORT=65000
NNODES=1
chmod +x /storage5/Megatron-LM/scripts/tsubame/moe_test_hatakeyama/1to2nodes/tanuki-4x8b_wo_zloss_best_fit.sh
...
for node in "${NODES[@]}"; do
    ...
    ssh -q $node "cd $SCRIPT_ROOT && \
        bash /storage5/Megatron-LM/scripts/tsubame/moe_test_hatakeyama/1to2nodes/tanuki-4x8b_wo_zloss_best_fit.sh $NODE_RANK
