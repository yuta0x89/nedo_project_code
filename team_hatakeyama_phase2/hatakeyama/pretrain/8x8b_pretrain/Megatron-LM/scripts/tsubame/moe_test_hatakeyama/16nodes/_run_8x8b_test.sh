...
NNODES=16
chmod +x /storage5/Megatron-LM/scripts/tsubame/moe_test_hatakeyama/16nodes/tanuki-8x8b_test.sh
...
for node in "${NODES[@]}"; do
    ...
    ssh -q $node "cd $SCRIPT_ROOT && \
        bash /storage5/Megatron-LM/scripts/tsubame/moe_test_hatakeyama/16nodes/tanuki-8x8b_test.sh $NODE_RANK
