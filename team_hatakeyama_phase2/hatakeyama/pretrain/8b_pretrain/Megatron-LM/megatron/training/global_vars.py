...
"""        
#0619マルチノード対応
#https://matsuokenllmcommunity.slack.com/archives/C06KPRNEK9U/p1718726011495429?thread_ts=1718712676.953749&cid=C06KPRNEK9U
def _set_wandb_writer(args):
    global _GLOBAL_WANDB_WRITER
    _ensure_var_is_not_initialized(_GLOBAL_WANDB_WRITER,
                                   'wandb writer')
    ...
    if getattr(args, 'wandb_project', '') and local_last_rank==0:
        ...
        now = datetime.now()
        now = now.strftime("%Y-%m-%d-%H-%M-%S")
        exp_name = args.wandb_name + "-" + now + "_" + str(args.rank)
        wandb_kwargs = {
            'entity': args.wandb_entity,
            'name': exp_name,
            'project': args.wandb_project,
...
def set_maintenance_detected_time(time: Union[float, None]) -> None:
    global _GLOBAL_MAINTENANCE_DETECTED_TIME
...
def get_maintenance_detected_time() -> Union[float, None]:
    global _GLOBAL_MAINTENANCE_DETECTED_TIME
