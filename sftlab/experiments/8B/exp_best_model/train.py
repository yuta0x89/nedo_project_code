...
class Preprocessor:
    ...
    def is_valid_text(self, text):
        """テキストが1文字以上の文字列であるかを確認する。
        Args:
            text (str): 確認するテキスト。
        Returns:
            bool: 有効なテキストであればTrue、そうでなければFalse。
        """
        return isinstance(text, str) and len(text) > 0

    def remove_stopwords(self, sample, stopwords):
        """サンプルからストップワードを取り除く。
        Args:
            sample (dict): 処理するデータを含むサンプル。
            stopwords (list): ストップワードのリスト。
        Returns:
            str: ストップワードを除いたテキスト。
        """
        pattern = '|'.join(re.escape(word) for word in stopwords)
        if sample is not None:
            sample[self.result_key] = re.sub(pattern, '', sample[self.result_key])
            return sample
        else:
            return None
    ...
    def apply_template(self, sample, instruction='instruction', input=None, output='output'):
        ...
        """
        if self.is_valid_text(sample[instruction]) and self.is_valid_text(sample[output]):
            # bos, eosトークンが入っていれば削除
            instruction_text = self.cleanup_text(sample[instruction])
            output_text = self.cleanup_text(sample[output])

            system_message = self.template_config['system']
            instruction_text = self.template_config['instruction'] + instruction_text
            result = system_message + instruction_text
            if input is not None and type(sample[input]) is str and len(sample[input]) > 0:
                # bos, eosトークンが入っていれば削除
                input_text = self.cleanup_text(sample[input])
                result += self.template_config['input'] + input_text
            result += self.template_config['output'] + output_text
    ...
    def preprocess_openai_messages(self, sample, messages, role='role', content='content', add_system_message=True, ignore_original_system_message=True, add_eos=False):
        ...
        for m in messages:
            if 'system' in m[role]:
                ...
                if not self.is_valid_text(m[content]):
                    return None
                turn += 1
                # bos, eosトークンが入っていれば削除
                instruction_text = self.cleanup_text(m[content])
                result.append(self.template_config['instruction'] + instruction_text)
            else:
                if not self.is_valid_text(m[content]):
                    return None
                # bos, eosトークンが入っていれば削除
                output_text = self.cleanup_text(m[content])
                response = self.template_config['output'] + output_text
    ...
    def preprocess_openai_messages_dpo_format(self, sample, messages, role='role', content='content', chosen='chosen', add_system_message=True, ignore_original_system_message=True, add_eos=False):
        """サンプル内のOpenAIメッセージを前処理してDPO用データからSFT用データを作成する。chosenで指定したキーを回答とする。
        Args:
            sample (dict): 処理するデータを含むサンプル。
            messages (str): サンプル内のメッセージのキー。
            role (str): メッセージ内の役割のキー。デフォルトは'role'。
        ...
        Returns:
            dict: 前処理されたメッセージを含む更新済みのサンプル。
        """
        sample = self.preprocess_openai_messages(sample, messages, role, content, add_system_message, ignore_original_system_message, add_eos)
        sample[self.result_key] = sample[self.result_key] + self.template_config['output'] + sample[chosen]
        return sample
    ...
    def preprocess_trl_with_score(self, sample, prompt='prompt', response1='response1', score1='score1', response2='response2', score2='score2'):
        ...
        if isinstance(sample['score1'], (int, float)) and isinstance(sample['score2'], (int, float)):
            ...
            sample[self.result_key] = sample['prompt'] + sample['chosen']
            return sample
...
def load_datasets(data_config_list, template_config, cache_dir, tokenizer, split):
    ...
    for data_config in data_config_list:
        if split in data_config['split']:
            dataset = load_dataset(data_config['name'], split=data_config['split'][split], cache_dir=cache_dir, ignore_verifications=True)
